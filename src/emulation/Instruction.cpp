...
Instruction::Instruction() : opcode(0), label("Not implemented!"), ops{ new NOP } {}
...
void Instruction::clock() {
    if (++clockCount < ops.at(instructionNum)->getLength()) { return; }
...
bool Instruction::isDone() { return instructionNum >= ops.size(); }

Instruction::~Instruction() {
    for (auto& operation : ops) { delete operation; }
}
