...
//  ImGuiConfigFlags_NoMouseCursorChange'. [X] Platform: Clipboard support. [X] Platform: Keyboard
//  arrays indexed using SDL_SCANCODE_* codes, e.g. ImGui::IsKeyPressed(SDL_SCANCODE_SPACE). [X]
...
// examples of using this. If you are new to Dear ImGui, read documentation from the docs/ folder +
// read the top of imgui.cpp. Read online: https://github.com/ocornut/imgui/tree/master/docs

// CHANGELOG
// (minor and older changes stripped away, please see git history for details)
//  2020-05-25: Misc: Report a zero display-size when window is minimized, to be consistent with
//  other backends. 2020-02-20: Inputs: Fixed mapping for ImGuiKey_KeyPadEnter (using
//  SDL_SCANCODE_KP_ENTER instead of SDL_SCANCODE_RETURN2). 2019-12-17: Inputs: On Wayland, use
//  SDL_GetMouseState (because there is no global mouse state). 2019-12-05: Inputs: Added support
//  for ImGuiMouseCursor_NotAllowed mouse cursor. 2019-07-21: Inputs: Added mapping for
//  ImGuiKey_KeyPadEnter. 2019-04-23: Inputs: Added support for SDL_GameController (if
...
// meant to be used by dear imgui, you may need to filter events based on their windowID field.
...
static void ImGui_ImplSDL2_UpdateMousePosAndButtons() {
    ...
    if (io.WantSetMousePos) SDL_WarpMouseInWindow(g_Window, (int)io.MousePos.x, (int)io.MousePos.y);
    else
...
#define MAP_BUTTON(NAV_NO, BUTTON_NO)                                                   \
    {                                                                                   \
        io.NavInputs[NAV_NO] =                                                          \
          (SDL_GameControllerGetButton(game_controller, BUTTON_NO) != 0) ? 1.0f : 0.0f; \
    }
#define MAP_ANALOG(NAV_NO, AXIS_NO, V0, V1)                                                     \
    {                                                                                           \
        float vn =                                                                              \
          (float)(SDL_GameControllerGetAxis(game_controller, AXIS_NO) - V0) / (float)(V1 - V0); \
        if (vn > 1.0f) vn = 1.0f;                                                               \
...
void ImGui_ImplSDL2_NewFrame(SDL_Window* window) {
    ...
    io.DeltaTime =
      g_Time > 0 ? (float)((double)(current_time - g_Time) / frequency) : (float)(1.0f / 60.0f);
    g_Time = current_time;
