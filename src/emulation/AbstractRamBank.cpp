...
bool AbstractRamBank::accepts(uint16_t addr) {
    return (addr >= addrLow && addr <= addrHigh);
}
uint16_t AbstractRamBank::getAddrStart() const {
    return addrLow;
}
std::unique_ptr<AbstractRamBank> AbstractRamBank::getEcho(uint16_t startAddr, std::weak_ptr<AbstractRamBank> echoedBank) {
