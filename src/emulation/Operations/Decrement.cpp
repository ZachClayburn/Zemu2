...
const constexpr static uint8_t HALF_CARY_VALUE{ 0xF0 };

Decrement::Decrement(CPURegisters* registersIn, Target targetIn)
  : Operation(LENGTH), target(targetIn), registers(registersIn) {}

interimValue_t Decrement::operator()(interimValue_t value) {
    uint8_t oldVal = 0;
    switch (target) {
    case A:
