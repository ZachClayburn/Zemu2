...
MainWindow::MainWindow() : screenBuffer(std::make_shared<std::array<Pixel, PIXEL_COUNT>>()) {
    ...
    texture = SDL_CreateTexture(
      renderer, SDL_PIXELFORMAT_RGB24, SDL_TEXTUREACCESS_STREAMING, GAMEBOY_WIDTH, GAMEBOY_HEIGHT);
...
void MainWindow::pause(uint32_t ms) { SDL_Delay(ms); }
void MainWindow::updateDisplay() {
    auto& window = MainWindow::get();
    SDL_UpdateTexture(
      window.texture, nullptr, window.screenBuffer->data(), GAMEBOY_WIDTH * sizeof(uint8_t) * 3);
